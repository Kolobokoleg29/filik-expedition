...
## P1 product, marketing and map asset checkpoint — 2026-09-10

- Добавлен docs/YANDEX_PRODUCT_BRIEF.md: продуктовая гипотеза, психологические принципы честной мотивации, воронка первого сеанса, тексты русского черновика и план метрик.
- Зафиксирован кандидат для черновика Яндекс Игр: Десктоп + Мобильные, мобильная Портретная ориентация, русский язык; публикация должна оставаться отложенной до ручной проверки SDK и материалов.
- Добавлен новый production-ассет assets/UI/map_route_compass.png: 384×384 прозрачный PNG, подключён через mapRouteCompass в manifest и использован как декоративный якорь заголовка карты.
- Добавлена адаптивная вёрстка заголовка карты и e2e-проверка загрузки компаса на мобильном и десктопном сценариях.
- Локальный полный gate после интеграции: syntax, content, 51 unit, build, budget, release-preflight и 15 Playwright-тестов проходят; замечание Invalid archive в текущем UI консоли не используется как доказательство ошибки проекта.
## P1 map state asset checkpoint — 2026-09-10

- Созданы и подключены `map_node_current.png`, `map_node_complete.png` и `map_node_locked.png`; каждый ассет используется в своей роли на карте, а не хранится как неиспользуемый вариант.
- Состояние уровня теперь читается через иллюстрацию и текстовый номер: текущий, завершённый и заблокированный узлы различаются формой/символом, а не только цветом.
- Новые PNG уменьшены до 192×192 и занимают 51–61 КБ каждый; общий asset budget остаётся в пределах лимитов.
- e2e asset regression проверяет 304 визуальных узла на desktop/mobile; полный локальный gate проходит.
## P1 Yandex draft metadata checkpoint — 2026-09-10

- Добавлен `docs/yandex-draft.json` как машинно-читаемый кандидат для русской карточки: тексты, теги, ключевые слова, портретная ориентация, cloud saves и отложенная публикация.
- Добавлен `scripts/check-yandex-draft.mjs` и npm-скрипт `test:product`; проверяются длины полей, lowercase keywords, количество тегов, русский язык и защитные настройки черновика.
- Проверка не изменяет кабинет и не заменяет ручную сверку подсказок консоли; сохранение внешних полей требует отдельного подтверждения пользователя.
## P1 Yandex console observation checkpoint — 2026-09-10

- Авторизованный черновик приложения 562137 просмотрен без изменения внешних полей: портретная ориентация и cloud save уже включены, русский язык и 0+ подтверждены.
- `Invalid archive` в блоке замечаний зафиксирован как известный UI-сигнал и исключён из критериев локальной готовности; локальный ZIP проходит `release:package`.
- Обнаружено расхождение версий: кабинет показывает 5.4.3, репозиторий и проверенный архив — 5.3.4. Версия и архив не подменяются автоматически; требуется осознанный шаг перед загрузкой.
## P1 release metadata gate checkpoint — 2026-09-10

- `release:package` теперь сначала запускает `test:product`, поэтому production ZIP нельзя собрать в обход проверки Yandex-метаданных.
- Локальный release-flow по-прежнему отделён от сохранения черновика: валидатор проверяет machine-readable кандидат, но не отправляет данные во внешний кабинет.
## P1 release consistency checkpoint — 2026-09-10

- `test:product` сверяет версию machine-readable черновика с `package.json`, чтобы подготовленные к консоли материалы не расходились с исходниками.
- `test:content` дополнен `check-asset-contracts.mjs`: четыре map PNG проверяются на существование, валидную PNG-сигнатуру, квадратность, размер до 512×512 и вес до 300 КБ.
## P1 visual and narrative shell checkpoint — 2026-09-10

- Добавлена спецификация визуальной системы в `docs/VISUAL_SYSTEM.md`: material language, плотность, mobile/desktop правила, inventory поверхностей и definition of done.
- История стала частью обычного маршрута: компактная запись текущей главы видна в лагере, сюжетная кнопка добавлена на карточки карты, а при входе в новую главу показывается chapter intro независимо от наличия спутника.
- Экран результата главы получил короткий recap записи и CTA «Читать запись главы».
- Story surface использует существующие chapter/artifact raster assets; новые декоративные SVG для этой итерации не добавлялись.
- E2E покрывает открытие истории из лагеря и chapter intro при пустой коллекции спутников. Следующий этап — покомпонентный визуальный pass игрового поля, карты, дневника, спутников, целей, магазина и сервисных окон с screenshot-регрессией.

## P1 surface-system checkpoint — 2026-09-10

- Общая модальная оболочка теперь маркируется `data-modal-type`, поэтому плотность, иерархию и состояния окон можно настраивать централизованно.
- Настройки получили компактные строки с единым ритмом, ежедневный маршрут — более плотную карточку этапов, цели/магазин/рейтинг — общую мобильную иерархию заголовков и акцентов.
- Акценты сохранены семантичными: морское стекло — действие и текущий путь, медь — находка и прогресс, красный — только недостаток или ошибка.
- Следующий этап — покомпонентный visual QA игрового поля, карты, дневника, спутников и окон магазина с реальными screenshot-базелайнами после восстановления внутреннего просмотра изображений.

## P1 map-navigation checkpoint — 2026-09-10

- На карте добавлено явное состояние текущей главы: карточка получает визуальный акцент, а прогресс показывает локализованную метку «Ваш маршрут».
- Верхняя навигация картографических, дневниковых и companion-страниц остаётся видимой при прокрутке, не занимая место в игровом поле.
- Сняты новые mobile/desktop screenshot-артефакты карты и игрового поля в `artifacts/`; полноценный визуальный просмотр отложен до восстановления внутреннего image viewer.
- E2E-покрытие проверяет единственную текущую главу и текущий уровень на карте; следующий срез — плотность карточек дневника и спутников, затем визуальная проверка реальными изображениями.

## P1 diary-navigation checkpoint — 2026-09-10

- Дневник использует тот же маршрутный сигнал, что и карта: следующая глава получает состояние «Ваш маршрут» и отдельный визуальный акцент.
- Запись текущей главы можно открыть до её завершения; завершённые предыдущие главы по-прежнему открывают страницу найденного артефакта.
- Для длинных коллекций включён `content-visibility: auto` с резервным размером карточки, чтобы 38 записей и 12 спутников не перегружали первый рендер.
- E2E проверяет монтирование дневника, текущую метку и открытие сюжетной записи. Следующий этап — финальный pass игровой HUD и состояний спутников после полного визуального просмотра screenshot-артефактов.

## P1 diary availability checkpoint — 2026-09-10

- Текущая глава в дневнике не маскируется под закрытую находку: у карточки убран замок и приглушение иллюстрации, а CTA сразу открывает сюжетную запись.
- Завершённые предыдущие главы сохраняют отдельный режим страницы артефакта, поэтому «текущий маршрут» и «коллекция находок» не смешиваются.
- E2E дополнительно проверяет отсутствие lock-иконки у доступной текущей записи и её фактическое открытие.

## P1 companion-state checkpoint — 2026-09-10

- В roster спутников активный попутчик получает отдельный статус «Сейчас с вами», teal-акцент карточки и точную семантику `aria-pressed`.
- Визуальный сигнал активного спутника отделён от locked-состояния и не требует читать длинное описание способности.
- E2E проверяет активную карточку, статус и доступное состояние действия выбора. Следующий срез — согласовать такие же state markers для готовых целей и купленных/повторяемых предложений магазина.

## P1 companion and service surfaces checkpoint — 2026-09-10

- Активный спутник получает отдельный status badge «Сейчас с вами», teal-рамку и `aria-pressed=true`, поэтому выбор читается до открытия подробностей.
- Цели и магазин проверены на мобильном viewport: hero, следующая цель, кошелёк и группы товаров доступны без горизонтального переполнения.
- Сняты desktop/mobile screenshot-артефакты спутников, целей и магазина в `artifacts/`; внутренний viewer пока не позволяет выполнить ручную визуальную оценку.
- Целевой UI/E2E-набор вырос до 14 зелёных сценариев. Следующий этап — проверить игровые state transitions: подсказки, бонусный сундук, завершение уровня и companion reactions.


## P1 gameplay feedback checkpoint — 2026-09-10

- Подсказка теперь кратко выделяет открытую клетку отдельной анимацией; правило списания монет/free hint и состав прогресса не менялись.
- Бонусная полоса получает разные короткие состояния для обычного бонусного слова и открытия сундука; расчёт награды остаётся прежним, а replay-режим не выдаёт экономическую награду и не показывает chest-state.
- Завершение уровня визуально фиксируется на игровом HUD до перехода в result modal, а старт активной экспедиции сопровождается репликой выбранного спутника с существующим aria-live="polite".
- Добавлены mobile e2e-проверки подсказки и реакции спутника; локальный gate после этапа: syntax, build, 51 unit и 16 Playwright-тестов проходят.
- Ручной visual review screenshot-артефактов по-прежнему отложен: внутренний image viewer в текущей сессии не поднимается.

## P1 purchase-pending checkpoint — 2026-09-10

- Добавлено единое pending-состояние для покупок через Yandex SDK: кнопка меняется на «Покупка…» или «Проверяем…», получает aria-busy=true и блокирует повторный клик до ответа SDK.
- Обработаны coin packs, «Убрать рекламу», стартовый набор и восстановление покупок; при cancel/error исходное состояние кнопки восстанавливается, а пользователь получает понятное сообщение.
