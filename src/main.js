...
const screens=createMetaScreens({
 app,state,setScreen:name=>{ui.screen=name;},getLevels:()=>levels,getConfig:()=>liveConfig,econ,weeklyCfg,rewardedCfg,rewardedRemaining,store,platform,analytics,leaveScreen,setBackground,topbar,btn,iconBtn,coinPill,chapterFor,chapterImage,chapterWideImage,petImage,petCard,captainTitle,captainMotto:captainMottoText,portraitIcon,captainAvatar,toast,save,modal,commerceAvailable,commerceProduct:id=>commerce.product(id),commerceBuy:id=>commerce.buy(id),commerceStatus:()=>commerce.catalogState,commerceRetry:()=>commerce.loadCatalog({force:true}),escapeHTML,startLevel,nextLevel,currentModal:()=>ui.modal
});
...
function startLevel(id,mode='campaign',overrideLevel=null){
 ...
 if(mode==='campaign'&&(id-1)%8===0&&!ui.replay&&state().activePet&&!(id===1&&!state().onboardingSeen))modal('chapter-welcome',`<p class="eyebrow">Глава ${chapter.id}</p><h2>${chapter.title}</h2>${petCard('expedition')}${btn('close','В путь','arrow','primary wide')}`);
}
...
function hint(word=false){if(!interactive())return;analytics.send('hint_opened',{level:ui.level.id,mode:ui.mode,type:word?'word':'letter'});const entries=ui.level.words.filter(w=>!ui.progress.found.includes(w.word));if(!entries.length)return;
 ...
 else{const cell=closed[Math.floor(Math.random()*closed.length)];ui.progress.revealed.push(cell.key);renderBoard();}
...
function modal(type,html,{close=true,center=false,cls=''}={}){
  ...
  const title=modalRoot.querySelector('h2');if(title)title.id='modal-title';
  queueMicrotask(()=>{const anchor=ui.focusAnchor?modalRoot.querySelector(ui.focusAnchor):null;ui.focusAnchor=null;const target=(anchor&&!anchor.disabled)?anchor:modalRoot.querySelector('[data-modal-focus]')||modalRoot.querySelector('button:not(:disabled)');target?.focus({preventScroll:true});});
}
function closeModal(restore=true){
  ...
  if(ui.modalStack.length&&restore){
    ...
    queueMicrotask(()=>{const f=modalRoot.querySelector('button:not(:disabled)');f?.focus({preventScroll:true});});
    return;
  }
...
async function action(name,id){if(ui.adRewardBusy)return;if(platform.paused&&!ui.modal&&name!=='close')return;if(['daily','album','pets','profile','weekly','leaderboard','goals','wallet','shop','endless'].includes(name)){analytics.send('feature_open',{feature:name});}
...
document.addEventListener('click',e=>{const el=e.target.closest('[data-action]');if(!el||el.disabled)return;if(platform.reasons.has('ad')||platform.reasons.has('platform')||platform.reasons.has('hidden'))return;audio.unlock();if(!['submit','shuffle','hint','word-hint'].includes(el.dataset.action))audio.play('click');void action(el.dataset.action,el.dataset.id);});
document.addEventListener('keydown',e=>{if(e.ctrlKey||e.altKey||e.metaKey)return;if(ui.modal){if(e.key==='Escape'){e.preventDefault();if(!ui.adRewardBusy&&ui.modalClosable)closeModal();}if(e.key==='Tab'){const list=[...modalRoot.querySelectorAll('button:not(:disabled),[href]')];if(!list.length)return;const i=list.indexOf(document.activeElement);if(e.shiftKey&&i<=0){e.preventDefault();list.at(-1).focus();}else if(!e.shiftKey&&i===list.length-1){e.preventDefault();list[0].focus();}}return;}
