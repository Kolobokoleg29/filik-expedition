...
const sdkScriptPath=(host,localProxy)=>{if(localProxy)return '/sdk.js';const hostname=String(host?.location?.hostname||'').toLowerCase();return /(^|\.)yandex\.[a-z.]+$/.test(hostname)?'/sdk.js':'';};
...
export class YandexPlatform {
  ...
  async init(){
    ...
    const scriptPath=sdkScriptPath(this.host,localProxy);
    if((!local||localProxy) && !this.host.YaGames && scriptPath){
      await timeout(new Promise(resolve=>{const s=this.host.document.createElement('script');s.src=scriptPath;s.onload=()=>{this.status('sdk','script_loaded');resolve();};s.onerror=()=>{this.status('sdk','script_error');resolve();};this.host.document.head.append(s);}),4000);
    }
